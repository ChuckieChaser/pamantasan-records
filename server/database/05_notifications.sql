...
CREATE POLICY notifications_select_access ON notifications FOR SELECT USING (is_system_role() OR recipient_id = get_user_current_id());
CREATE POLICY notifications_insert_access ON notifications FOR INSERT WITH CHECK (is_system_role());
CREATE POLICY notifications_update_access ON notifications FOR UPDATE USING (is_system_role() OR recipient_id = get_user_current_id()) WITH CHECK (is_system_role() OR recipient_id = get_user_current_id());
CREATE POLICY notifications_delete_access ON notifications FOR DELETE USING (is_system_role() OR recipient_id = get_user_current_id());