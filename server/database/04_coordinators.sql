...
CREATE POLICY coordinator_requests_select_access ON coordinator_requests FOR SELECT USING (is_system_role() OR is_administrator_role() OR requester_id = get_user_current_id());
CREATE POLICY coordinator_requests_insert_access ON coordinator_requests FOR INSERT WITH CHECK (is_system_role() OR (is_coordinator_role() AND requester_id = get_user_current_id()));
CREATE POLICY coordinator_requests_update_access ON coordinator_requests FOR UPDATE USING (is_system_role() OR is_administrator_role()) WITH CHECK (is_system_role() OR is_administrator_role());
CREATE POLICY coordinator_requests_delete_access ON coordinator_requests FOR DELETE USING (is_system_role() OR (requester_id = get_user_current_id() AND status = 'PENDING'));