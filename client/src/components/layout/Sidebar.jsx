    ...
    return (
        <aside className="flex h-full w-20 shrink-0 flex-col items-center justify-between border-r border-border bg-surface py-6">
            ...
            <nav className="flex flex-col items-center gap-4">
                ...
                {user.role === USERS_ROLE.ADMINISTRATOR && <NavigationButton icon={Users} active={isActive('/management')} onClick={() => navigate('/management')} title="Management" />}
                ...
                {user.role !== USERS_ROLE.ADMINISTRATOR && user.role !== USERS_ROLE.COORDINATOR && <NavigationButton icon={ClipboardList} active={isActive('/request')} onClick={() => navigate('/request')} title="Requests" />}
            </nav>
            ...
            {/* ========================================= */}
            <div className="flex flex-col items-center gap-4">
                <ImageButton src={user?.avatar_path ?? avatar} alt="User Profile" size="medium" />
            </div>
        </aside>
    );
}
