...
-- Versions RLS
CREATE POLICY document_versions_select_access ON document_versions FOR SELECT USING (is_system_role() OR is_administrator_role() OR EXISTS (SELECT 1 FROM documents d WHERE d.id = document_versions.document_id));
CREATE POLICY document_versions_insert_access ON document_versions FOR INSERT WITH CHECK (is_system_role() OR is_administrator_role());
...
CREATE POLICY document_requests_update_access ON document_requests FOR UPDATE USING (is_system_role() OR is_administrator_role()) WITH CHECK (is_system_role() OR is_administrator_role());
...
-- Messages RLS
CREATE POLICY document_request_messages_select_access ON document_request_messages FOR SELECT USING (is_system_role() OR is_administrator_role() OR EXISTS (SELECT 1 FROM document_requests dr WHERE dr.id = document_request_messages.document_request_id AND dr.requester_id = get_user_current_id()));
CREATE POLICY document_request_messages_insert_access ON document_request_messages FOR INSERT WITH CHECK (is_system_role() OR (user_id = get_user_current_id() AND EXISTS (SELECT 1 FROM document_requests dr WHERE dr.id = document_request_messages.document_request_id AND dr.status = 'OPEN' AND (is_administrator_role() OR dr.requester_id = get_user_current_id()))));

-- Shares RLS
CREATE POLICY document_shares_select_access ON document_shares FOR SELECT USING (is_system_role() OR is_administrator_role() OR ((is_director_role() OR is_officer_role()) AND department_id = get_user_current_department_id()) OR sharer_id = get_user_current_id() OR recipient_id = get_user_current_id() OR department_id = get_user_current_department_id() OR EXISTS (SELECT 1 FROM document_requests dr WHERE dr.id = document_shares.document_request_id AND dr.requester_id = get_user_current_id()));
...
-- Documents RLS
CREATE POLICY documents_select_access ON documents FOR SELECT USING (is_system_role() OR is_administrator_role() OR (status != 'ARCHIVED' AND (uploader_id = get_user_current_id() OR EXISTS (SELECT 1 FROM document_versions dv WHERE dv.document_id = documents.id AND dv.rejecter_id = get_user_current_id()) OR EXISTS (SELECT 1 FROM document_shares ds WHERE ds.document_id = documents.id AND ((ds.department_id = get_user_current_department_id() AND ((status IN ('PENDING_OFFICER', 'PENDING_DIRECTOR', 'PUBLISHED') AND is_officer_role()) OR (status IN ('PENDING_DIRECTOR', 'PUBLISHED') AND is_director_role()) OR (status = 'PUBLISHED' AND is_member_role() AND (ds.recipient_id IS NULL OR ds.recipient_id = get_user_current_id())))) OR (status = 'ATTACHMENT' AND ds.document_request_id IS NOT NULL AND EXISTS (SELECT 1 FROM document_requests dr WHERE dr.id = ds.document_request_id AND dr.requester_id = get_user_current_id())))))));
CREATE POLICY documents_insert_access ON documents FOR INSERT WITH CHECK (is_system_role() OR is_administrator_role());
CREATE POLICY documents_update_access ON documents FOR UPDATE USING (is_system_role() OR is_administrator_role() OR (EXISTS (SELECT 1 FROM document_shares ds WHERE ds.document_id = documents.id AND ds.department_id = get_user_current_department_id()) AND ((status = 'PENDING_OFFICER' AND is_officer_role()) OR (status = 'PENDING_DIRECTOR' AND is_director_role())))) WITH CHECK (is_system_role() OR (is_administrator_role() AND (status != 'ARCHIVED' OR NOT EXISTS (SELECT 1 FROM document_shares ds WHERE ds.document_id = id) OR EXISTS (SELECT 1 FROM documents d WHERE d.id = id AND d.status = 'ATTACHMENT'))) OR (EXISTS (SELECT 1 FROM document_shares ds WHERE ds.document_id = documents.id AND ds.department_id = get_user_current_department_id()) AND ((status = 'PENDING_OFFICER' AND is_officer_role()) OR (status = 'PENDING_DIRECTOR' AND is_director_role()))));
